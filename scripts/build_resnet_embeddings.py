"""Generate full ResNet-50 embeddings and build Database-ID-aware FAISS index for Week 3.

Uses EXACTLY the same catalog rows and stable catalog_item IDs as CLIP.
...
Outputs:
  - artifacts/embeddings/resnet_embeddings.npy
  - artifacts/embeddings/resnet_catalog_ids.npy
  - artifacts/faiss/resnet.index
  - artifacts/faiss/resnet_mapping.csv
  - artifacts/embeddings/resnet_fingerprint.json
"""
...
CLIP_CATALOG_IDS_PATH = PROJECT_ROOT / "artifacts" / "embeddings" / "clip_catalog_ids.npy"

EMBEDDINGS_DIR = PROJECT_ROOT / "artifacts" / "embeddings"
FAISS_DIR = PROJECT_ROOT / "artifacts" / "faiss"

OUTPUT_EMBEDDINGS = EMBEDDINGS_DIR / "resnet_embeddings.npy"
OUTPUT_CATALOG_IDS = EMBEDDINGS_DIR / "resnet_catalog_ids.npy"
OUTPUT_FINGERPRINT = EMBEDDINGS_DIR / "resnet_fingerprint.json"
OUTPUT_FAISS_INDEX = FAISS_DIR / "resnet.index"
OUTPUT_RESNET_MAPPING = FAISS_DIR / "resnet_mapping.csv"
...
def parse_args() -> argparse.Namespace:
    """Parse CLI arguments."""
    parser = argparse.ArgumentParser(description="Generate full ResNet-50 embeddings and FAISS index.")
    parser.add_argument(
        "--catalog-csv",
        type=Path,
...
def compute_dataset_fingerprint(catalog_csv_path: Path, id_map_csv_path: Path) -> str:
    ...
    with open(id_map_csv_path, "rb") as f:
        while chunk := f.read(65536):
            hasher.update(chunk)
    hasher.update(f"{MODEL_NAME}:{WEIGHTS_NAME}:{EMBEDDING_DIMENSION}".encode("utf-8"))
...
def generate_resnet_embeddings(
    ...
    """Generate normalized ResNet-50 embeddings and build FAISS IndexIDMap2."""
    start_time = time.time()
    print("=" * 70, flush=True)
    print(" " * 12 + "RESNET-50 EMBEDDING & FAISS INDEX BUILDER", flush=True)
    print("=" * 70, flush=True)
    ...
    print(f"Pretrained Weights:       {WEIGHTS_NAME}", flush=True)
    ...
    ckpt_emb_path = EMBEDDINGS_DIR / "resnet_embeddings_checkpoint.npy"
    ckpt_ids_path = EMBEDDINGS_DIR / "resnet_catalog_ids_checkpoint.npy"
    ckpt_fp_path = EMBEDDINGS_DIR / "resnet_checkpoint_fingerprint.json"
    ...
    if not force_recompute and OUTPUT_EMBEDDINGS.exists() and OUTPUT_CATALOG_IDS.exists() and OUTPUT_FINGERPRINT.exists():
        ...
        if fp_data.get("fingerprint") == current_fp:
            print(f"Found completed valid ResNet embeddings ({OUTPUT_EMBEDDINGS.name}). Loading...", flush=True)
    ...
    # 3. Load ResNet-50 feature extractor
    print(f"\nLoading ResNet-50 ({WEIGHTS_NAME}) onto {device}...", flush=True)
    ...
    print(f"\nExtracting ResNet-50 embeddings for {len(remaining_records):,} remaining images...", flush=True)
    ...
    print(f"\nSaved final ResNet embeddings:", flush=True)
    ...
    print(f"\nResNet embeddings & FAISS index completed in {total_elapsed:.2f} seconds.", flush=True)
    print("=" * 70, flush=True)
...
def build_and_save_faiss_index(
    ...
    print("\nConstructing FAISS IndexIDMap2(IndexFlatIP) for ResNet-50...", flush=True)
    ...
    df_resnet_mapping = pd.DataFrame(mapping_records)
    df_resnet_mapping.to_csv(OUTPUT_RESNET_MAPPING, index=False)
    print(f"  + Wrote {len(df_resnet_mapping):,} rows to: {OUTPUT_RESNET_MAPPING.relative_to(PROJECT_ROOT)}", flush=True)

    # 8. HARD FAIR-COMPARISON ASSERTIONS
    print("\nRunning Full Verification & Fair-Comparison Assertions...", flush=True)
    ...
    assert len(df_resnet_mapping) == total_expected, "ResNet mapping rows mismatch!"
    print(f"  [PASS] Counts match exactly ({id_index.ntotal:,} == {total_expected:,}).", flush=True)

    # Assertion 2: EXACT ID Set Match with CLIP catalog IDs (CRITICAL FAIR COMPARISON)
    if CLIP_CATALOG_IDS_PATH.exists():
        clip_ids = np.load(str(CLIP_CATALOG_IDS_PATH))
        assert set(clip_ids) == set(catalog_ids), (
            "FATAL FAIR-COMPARISON ERROR: set(CLIP catalog IDs) != set(ResNet catalog IDs)!"
        )
        print(f"  [PASS] CRITICAL: set(CLIP catalog IDs) == set(ResNet catalog IDs) (Exact 100% Set Equality).", flush=True)
    ...
    for fid in sample_faiss_ids:
        ...
        map_rec = df_resnet_mapping[df_resnet_mapping["faiss_id"] == fid].iloc[0]
    ...
    print("\nExecuting ResNet Search Query Verification Test...", flush=True)
